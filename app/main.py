...
from .models.response import PredictionResponse, ResponseMetadata, ModelResult
...
async def predict(request: PredictionRequest):
    ...
    try:
        ...
        request_timestamp = request.event_timestamp or get_current_timestamp_ms()
        ...
        if not entity_ids:
            return PredictionResponse(
                metadata=ResponseMetadata(models_name=request.models),
                results=[]
            )
        ...
        if not request.models:
            ...
            return PredictionResponse(
                metadata=ResponseMetadata(models_name=[]),
                results=results
            )
        ...
        response_timestamp = request_timestamp + processing_time_ms
        
        # Format results
        formatted_results = []
        for entity_result in prediction_results:
            timestamps = [response_timestamp] * len(request.models)
            formatted_results.append(ModelResult(
